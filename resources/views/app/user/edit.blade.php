...
@section('breadcrumb')
	...
	<li>
		<a href="{{ route('app.settings.users.edit', array('id'=>$user->id)) }}">Editar</a>
		<i class="fa fa-circle"></i>
	</li>
@endsection
	...
	<div class="page-content-inner">
		<div class="portlet light ">
			...
			<div class="portlet-body form">
				<form class="form-horizontal" role="form" method="post" action="{{ route('app.settings.users.update', array('id'=>$user->id)) }}">
					{{ csrf_field() }}
					<div class="form-body">
							...
							<div class="col-md-9">
								<div class="input-inline input-medium">
									<div class="input-group">
										...
										<input type="text" class="form-control" placeholder="Usuario" name="name" value="{{ $user->name }}">
									</div>
								</div>
							...
							<div class="col-md-9">
								<select multiple="multiple" class="multi-select" id="permissions" name="permissions[]">
									@foreach($permissions as $permission)
										<option value="{{ $permission->id }}" {{ ($user->can($permission->name)?'selected':'') }}>{{ $permission->display_name }}</option>
									@endforeach
								</select>
							</div>
						</div>
